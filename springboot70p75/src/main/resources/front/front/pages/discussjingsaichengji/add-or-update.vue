	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjingsaichengji`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjingsaichengji`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjingsaichengji`, this.ruleForm);
						}else{
							await this.$api.add(`discussjingsaichengji`, this.ruleForm);
						}
						uni.setStorageSync('discussjingsaichengjiCleanType',true);
