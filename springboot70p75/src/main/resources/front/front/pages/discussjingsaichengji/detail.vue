...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjingsaichengjiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjingsaichengjiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjingsaichengji');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussjingsaichengji');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjingsaichengji', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjingsaichengji', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussjingsaichengji/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjingsaichengji', this.detail);
				this.$utils.msg('审核成功');
