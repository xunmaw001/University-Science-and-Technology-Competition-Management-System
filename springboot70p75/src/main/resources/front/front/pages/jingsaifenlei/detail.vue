...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jingsaifenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jingsaifenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jingsaifenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjingsaifenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jingsaifenlei', this.detail);
				this.$utils.msg('审核成功');
