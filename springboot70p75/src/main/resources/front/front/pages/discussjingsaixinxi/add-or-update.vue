	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjingsaixinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjingsaixinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjingsaixinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussjingsaixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussjingsaixinxiCleanType',true);
